...
```bash
# Preferred: push tag vX.Y.Z and download the release artifact generated by
# .github/workflows/release.yml

# See docs/RELEASING.md for the exact tag creation and push steps.

# Local fallback: build and package the canonical store artifact yourself
npm run package:release

# Verify the zip
unzip -l planmypeak-importer-webstore-vX.Y.Z.zip
```
...
2. Click **"New Item"**
3. Upload `planmypeak-importer-webstore-vX.Y.Z.zip`
...
```bash
# Update package.json and public/manifest.json to the release version first,
# commit, and tag it as vX.Y.Z.

# Local fallback if you are not using the release workflow
npm run package:release
