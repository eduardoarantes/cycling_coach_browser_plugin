...
- **Confirmed**: Production Supabase project URL is `https://nwvtltfibnkdogdeeluh.supabase.co` (replaces the retired project). Still to confirm during implementation: the production anon key value and whether the `apikey` header is observable on portal traffic.
